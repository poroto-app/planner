...
type PlanCandidateEntity struct {
	...
	Plans                         []PlanInternalCandidateEntity `firestore:"plans"`
...
func ToPlanCandidateEntity(planCandidate models.PlanCandidate) PlanCandidateEntity {
	plans := make([]PlanInternalCandidateEntity, len(planCandidate.Plans))
	for i, plan := range planCandidate.Plans {
		plans[i] = toPlanCandidateEntity(
			plan.Id,
			plan.Name,
			plan.Places,
			plan.TimeInMinutes,
...
func FromPlanCandidateEntity(entity PlanCandidateEntity) models.PlanCandidate {
	...
	for i, plan := range entity.Plans {
		plans[i] = fromPlanInternalCandidateEntity(
			plan.Id,
			plan.Name,
			plan.Places,
			plan.TimeInMinutes,
