...
func TestFlatMap(t *testing.T) {
	cases := []struct {
		...
		{
			name:      "empty slice",
			transform: func(i int) []string { return []string{strconv.Itoa(i)} },
			slice:     []int{},
			expected:  []string{},
		},
		{
		...
		{
			name:      "multiple elements",
			transform: func(i int) []string { return []string{strconv.Itoa(i), strconv.Itoa(i)} },
			slice:     []int{1, 2, 3},
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			actual := FlatMap(c.slice, c.transform)
			if len(actual) != len(c.expected) {
				t.Errorf("expected: %v, actual: %v", c.expected, actual)
			}
			for i, v := range actual {
