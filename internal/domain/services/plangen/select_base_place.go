...
func (s Service) SelectBasePlace(input SelectBasePlaceInput) []models.Place {
	...
	// すでに選択された場所は除外
	places = array.Filter(places, func(place models.Place) bool {
	...
	s.logger.Debug("Places after filtering ignore places", zap.Int("Places", len(places)))
	...
	for len(placesSelected) < input.MaxBasePlaceCount || len(places) > 0 {
		...
		s.logger.Debug("Places after filtering by distance from selected places", zap.Int("Places", len(places)))

		// レビューの最も高い場所を選択する
		placesSortedByRating := models.SortPlacesByRating(places)
