package models

import "testing"

func GetPlaceTest(t *testing.T) {
	cases := []struct {
		name     string
		plan     Plan
		placeId  string
		expected *Place
	}{
		{
			name: "Valid place ID",
			plan: Plan{
				Places: []Place{
					{
						Id:   "1",
						Name: "place1",
					},
				},
			},
			placeId: "1",
			expected: &Place{
				Id:   "1",
				Name: "place1",
			},
		},
		{
			name: "Invalid place ID",
			plan: Plan{
				Places: []Place{
					{
						Id:   "1",
						Name: "place1",
					},
				},
			},
			placeId:  "2",
			expected: nil,
		},
	}

	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			actual := c.plan.GetPlace(c.placeId)
			if actual != c.expected {
				t.Errorf("expected: %v, actual: %v", c.expected, actual)
			}
		})
	}
}

func GetTransitionTest(t *testing.T) {
	cases := []struct {
		name             string
		plan             Plan
		placeId          string
		expectedPlace    *Place
		expectedDuration uint
	}{
		{
			name: "Valid place ID",
			plan: Plan{
				Places: []Place{
					{
						Id:   "1",
						Name: "place1",
					},
					{
						Id:   "2",
						Name: "place2",
					},
				},
				Transitions: []Transition{
					{
						FromPlaceId: "1",
						ToPlaceId:   "2",
						Duration:    10,
					},
				},
			},
			placeId: "1",
			expectedPlace: &Place{
				Id:   "2",
				Name: "place2",
			},
			expectedDuration: 10,
		},
		{
			name: "Invalid place ID",
			plan: Plan{
				Places: []Place{
					{
						Id:   "1",
						Name: "place1",
					},
					{
						Id:   "2",
						Name: "place2",
					},
				},
				Transitions: []Transition{
					{
						FromPlaceId: "1",
						ToPlaceId:   "2",
						Duration:    10,
					},
				},
			},
			placeId:          "3",
			expectedPlace:    nil,
			expectedDuration: 0,
		},
	}

	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			actualPlace, actualDuration, err := c.plan.GetTransition(c.placeId)
			if err != nil {
				t.Errorf("expected: %v, actual: %v", c.expectedPlace, actualPlace)
			}
			if actualPlace != c.expectedPlace {
				t.Errorf("expected: %v, actual: %v", c.expectedPlace, actualPlace)
			}
			if actualDuration != c.expectedDuration {
				t.Errorf("expected: %v, actual: %v", c.expectedDuration, actualDuration)
			}
		})
	}
}
